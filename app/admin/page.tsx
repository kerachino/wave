...
type AdminTab = "chat" | "purchase" | "profile";
...
function PurchasePanel({ order, selectedIds, setSelectedIds, customOptions, setCustomOptions, note, setNote, amounts, saving, message, approve }: { order: Order | null; selectedIds: string[]; setSelectedIds: React.Dispatch<React.SetStateAction<string[]>>; customOptions: CustomOption[]; setCustomOptions: React.Dispatch<React.SetStateAction<CustomOption[]>>; note: string; setNote: (value: string) => void; amounts: ReturnType<typeof computeAmounts>; saving: boolean; message: string; approve: () => Promise<void> }) { return <section className="mt-6 rounded-3xl border border-line bg-white p-6 shadow-card sm:p-8"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold tracking-[0.14em] text-brand">PURCHASE CONTROL</p><h3 className="mt-2 font-maru text-xl font-bold text-ink">購入許可の設定</h3></div>{order && <span className="rounded-full bg-cream px-3 py-1 text-xs text-ink-soft">{order.paymentStatus}</span>}</div><div className="mt-5 space-y-3">{purchasableOptions.map((option) => <label key={option.id} className="flex justify-between rounded-xl border border-line bg-cream px-4 py-3 text-sm"><span><input type="checkbox" checked={selectedIds.includes(option.id)} onChange={(event) => setSelectedIds((current) => event.target.checked ? [...current, option.id] : current.filter((id) => id !== option.id))} className="mr-3" />{option.name}</span><strong>{yen(option.defaultPrice)}</strong></label>)}</div><div className="mt-6 rounded-2xl border border-dashed border-line p-4"><div className="flex items-center justify-between gap-3"><div><h4 className="text-sm font-bold text-ink">自由追加項目</h4><p className="mt-1 text-xs text-ink-mute">定型外の内容も名称と価格を自由に設定できます。</p></div><button type="button" onClick={() => setCustomOptions((current) => [...current, { id: `custom-${Date.now()}`, name: "", price: 0, defaultPrice: 0 }])} className="rounded-full border border-brand px-3 py-2 text-xs font-bold text-brand">項目を追加</button></div><div className="mt-3 space-y-2">{customOptions.map((option) => <div key={option.id} className="flex gap-2"><input value={option.name} onChange={(event) => setCustomOptions((current) => current.map((item) => item.id === option.id ? { ...item, name: event.target.value } : item))} placeholder="項目名" className="min-w-0 flex-1 rounded-lg border border-line bg-cream px-3 py-2 text-sm" /><input type="number" min="0" value={option.price || ""} onChange={(event) => setCustomOptions((current) => current.map((item) => item.id === option.id ? { ...item, price: Number(event.target.value), defaultPrice: Number(event.target.value) } : item))} placeholder="価格" className="w-28 rounded-lg border border-line bg-cream px-3 py-2 text-sm" /><button type="button" onClick={() => setCustomOptions((current) => current.filter((item) => item.id !== option.id))} className="rounded-lg px-2 text-sm text-ink-mute hover:bg-cream" aria-label="自由追加項目を削除">×</button></div>)}</div></div><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} placeholder="運営者メモ（任意）" className="mt-5 w-full rounded-xl border border-line bg-cream px-4 py-3 text-sm" /><div className="mt-5 rounded-2xl bg-brand px-5 py-4 text-sm text-white"><div className="flex justify-between"><span>合計</span><strong>{yen(amounts.totalPrice)}</strong></div><div className="mt-1 flex justify-between"><span>初回支払い（50%）</span><strong>{yen(amounts.firstAmount)}</strong></div></div><div className="mt-5 flex flex-wrap items-center gap-4"><button onClick={approve} disabled={saving} className="rounded-full bg-brand px-6 py-3 text-sm font-bold text-white disabled:opacity-50">{saving ? "保存中..." : "購入を許可する"}</button>{message && <p className="text-sm text-ink-soft">{message}</p>}</div></section>; }
